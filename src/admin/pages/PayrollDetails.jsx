...
function PayrollView() {
  ...
  const locationBreakdown = step5.location_breakdown || step2.location_breakdown || [];
  
  // Get totals from step_5 or step_2
  const totalEarnings = step5.gross_salary || step2.total_earnings || payroll.gross_salary || 0;
  const totalDeductions = step5.deductions || step4.total_deductions || payroll.deductions || 0;
  const netPay = step5.net_pay || payroll.net_pay || 0;
  
  // Get worked days from step_1
  const totalWorkedDays = stepData.step_1?.days_present || stepData.step_1?.total_working_days || 0;
  const totalDays = stepData.step_1?.total_working_days || 30;
  
  // Get overtime from step_5 or step_3
  const overtimeDetails = step5.overtime_details || stepData.step_3?.overtime_details || [];
  const totalOvertime = step5.overtime || stepData.step_3?.total_overtime_amount || 0;
  ...
  const joiningDate = payroll.joining_date || employeeDetails?.joining_date || employeeDetails?.date_of_joining || "-";
  
  // ✅ Get bank details from payroll data (bank_details array) or employeeDetails
  const bankDetails = payroll.bank_details || employeeDetails?.bank_details || [];
  const primaryBank = bankDetails.length > 0 ? bankDetails[0] : null;
  
  const bankName = primaryBank?.bank_name || employeeDetails?.bank_name || "-";
  const accountNumber = primaryBank?.account_number || employeeDetails?.account_number || "-";
  const swiftCode = primaryBank?.swift_code || employeeDetails?.swift_code || employeeDetails?.ifsc_code || "-";
  const branchName = primaryBank?.branch_name || employeeDetails?.branch || "N/A";
  ...
  return (
    <div className="w-full overflow-x-hidden px-4 md:px-6 py-6">
      ...
      <div className="flex flex-wrap justify-end gap-3 mb-6">
        ...
        >
          <i className={`fas ${actionLoading ? "fa-spinner fa-spin" : "fa-paper-plane"}`}></i>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xl overflow-hidden max-w-5xl mx-auto">
        ...
        <div className="px-6 md:px-11 py-7 md:py-9">
          {/* Status Badge */}
          <div className="mb-6">
            <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-semibold ${getStatusBadge(payroll.status)}`}>
              <i className={`fas ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6">
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 md:p-6">
              ...
              <table className="w-full border-collapse text-sm">
                <tbody>
                  ...
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">Designation</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right">
                      {designation}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">Department</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right">
                      {department}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">Date of Joining</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right">
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 md:p-6">
              ...
              <table className="w-full border-collapse text-sm">
                <tbody>
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">Bank Name</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right">
                      {bankName}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">Account Number</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right">
                      {accountNumber}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">IBAN</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right text-xs">
                      {ibanNumber}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">SWIFT / IFSC</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right">
                      {swiftCode}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">Branch</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right">
                      {branchName}
                    </td>
                  </tr>
                  <tr>
                    <td className="py-1.5 text-gray-500 dark:text-gray-400 font-medium">Bank Country</td>
                    <td className="py-1.5 text-gray-800 dark:text-gray-200 font-semibold text-right">
                      {bankCountry}
                    </td>
                  </tr>
                ...
                <div>
                  ...
                  {step5.conversion_rates && (
                    <div className="flex flex-wrap gap-2 mt-1">
                      {Object.entries(step5.conversion_rates).map(([currency, rate]) => (
                ...
                <tbody>
                  {locationBreakdown.length > 0 ? (
                    locationBreakdown.map((loc, locIndex) => {
                      const locCurrency = loc.currency || targetCurrencyDisplay;
                      const hasEarnings = loc.salary_components && loc.salary_components.some(c => c.amount > 0);
                      
                      return (
                        <React.Fragment key={locIndex}>
                          <tr className="bg-emerald-50 dark:bg-emerald-900/20">
                            <td colSpan="2" className="px-4 py-2 text-emerald-700 dark:text-emerald-400 font-bold text-xs">
                              {loc.location_name || "Package"} ({loc.worked_days || 0} Days) - {locCurrency}
                            </td>
                          </tr>
                          {loc.salary_components && loc.salary_components.length > 0 && hasEarnings ? (
                            loc.salary_components.map((comp, compIndex) => (
                              <tr key={compIndex} className="border-b border-gray-100 dark:border-gray-700/50">
                            ...
                            <tr>
                              <td colSpan="2" className="px-4 py-2 pl-8 text-gray-400 dark:text-gray-500 italic">
                    ...
                    deductionsDetails.map((d, index) => (
                      <tr key={index} className="border-b border-gray-100 dark:border-gray-700/50">
                        ...
                        <td className="px-4 py-2 text-right text-red-600 dark:text-red-400 font-semibold">
                          {formatCurrency(d.amount, d.currency || targetCurrencyDisplay)}
                        </td>
                      </tr>
                    ))
                  ) : (
          ...
          {/* Overtime (if any) */}
          {overtimeDetails && overtimeDetails.length > 0 && totalOvertime > 0 && (
            <div className="mb-6">
              <div className="text-[10px] md:text-xs font-extrabold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider mb-3">
                Overtime Details
              </div>
                    ...
                    {overtimeDetails.map((ot, index) => (
                      <tr key={index} className="border-b border-gray-100 dark:border-gray-700/50">
                        ...
                        <td className="px-4 py-2 text-right text-orange-600 dark:text-orange-400 font-semibold">
                          {formatCurrency(ot.amount || 0, ot.currency || targetCurrencyDisplay)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <div className="bg-gradient-to-r from-emerald-900 to-emerald-700 dark:from-emerald-800 dark:to-emerald-900 rounded-xl p-5 md:p-7 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-lg">
            <div>
              ...
              <div className="text-xs text-white/60 mt-1">
                Disbursed to {bankName || "registered bank"} - Account {accountNumber || "registered account"}
              </div>
